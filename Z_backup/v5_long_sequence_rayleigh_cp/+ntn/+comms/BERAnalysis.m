        ...
        %   chEst : ntn.comms.ChannelEstimator
            ...
            for ki = 1:numel(obj.EbN0_dB)
                ...
                for trial = 1:obj.N_trials
                    ...
                    % Build continuous transmit sequences with CP
                    % Slot size with CP is ND + Ncp
                    % Block size with CP is 2 * (ND + Ncp)
                    L_block = 2 * (p.ND + p.Ncp);
                    xD_tx_DU_flat = zeros(L_block * obj.M_seq, 1);
                    
                    for m = 1:obj.M_seq
                        x_S_cp = [p.s_mls(end - p.Ncp + 1 : end); p.s_mls];
                        x_D_cp = [p.s_mls(end - p.Ncp + 1 : end) * d_sim(m); p.s_mls * d_sim(m)];
                        xD_tx_DU_flat((m-1)*L_block + 1 : m*L_block) = [x_S_cp; x_D_cp];
                    end
                    
                    % We do the same for the target scattered path (using the same transmit codes)
                    xD_tx_DTU_flat = xD_tx_DU_flat;

                    % Generate continuous received signal with time-varying delay
                    y_rx_flat = zeros(L_block * obj.M_seq, 1);
                    for m = 1:obj.M_seq
                        ...
                        % Shift the entire continuous sequences
                        xD_del_DU  = circshift(xD_tx_DU_flat,  ell_du);
                        xD_del_DTU = circshift(xD_tx_DTU_flat, ell_dtu);
                        
                        % For block 1, clear the startup transient
                        if m == 1
                            xD_del_DU(1 : ell_du) = 0;
                            xD_del_DTU(1 : ell_dtu) = 0;
                        end
                        
                        y_rx_flat(blk_start:blk_end) = ...
                            A_DU_m(m)  * xD_del_DU(blk_start:blk_end)  * ph_DU  ...
                          + A_DTU_m(m) * xD_del_DTU(blk_start:blk_end) * ph_DTU ...
                    ...
                    % Reshape to blocks
                    y_full_sim = reshape(y_rx_flat_aligned, [L_block, obj.M_seq]);
                    
                    % Split into sounding and data slots
                    yS_cp = y_full_sim(1 : p.ND + p.Ncp, :);
                    yP_cp = y_full_sim(p.ND + p.Ncp + 1 : end, :);
                    
                    % Discard the first Ncp samples of each slot (CP removal)
                    yS = yS_cp(p.Ncp + 1 : end, :);
                    yP = yP_cp(p.Ncp + 1 : end, :);
