...
for ki = 1:numel(EbN0_dB)
    ...
    for trial = 1:N_trials
        ...
        % Build continuous received signal block-by-block
        y_rx_flat = zeros(2*params.ND * M_seq, 1);
        for m = 1:M_seq
            blk_start = (m-1)*2*params.ND + 1;
            blk_end   = m*2*params.ND;
            
            x_block = [params.s_mls; params.s_mls * d_sim(m)];
            x_del_DU = circshift(x_block, mob.ell_DU_seq(m));
            ph_DU = exp(1j*2*pi*mob.fD_DU_seq(m) * (m-1) * params.Tblock);
            
            y_rx_flat(blk_start:blk_end) = ...
                A_DU_m(m) * x_del_DU * ph_DU ...
              + sigU * (randn(2*params.ND, 1) + 1j*randn(2*params.ND, 1));
        end
        
        % Frame synchronization: shift back by initial delay
        y_rx_flat_aligned = circshift(y_rx_flat, [-ell_sync, 0]);
        y_full_sim = reshape(y_rx_flat_aligned, [2*params.ND, M_seq]);
