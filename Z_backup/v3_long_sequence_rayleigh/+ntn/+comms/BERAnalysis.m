        ...
        %   chEst : ntn.comms.ChannelEstimator
            ...
            for ki = 1:numel(obj.EbN0_dB)
                ...
                for trial = 1:obj.N_trials
                    ...
                    for m = 1:obj.M_seq
                        blk_start = (m-1)*2*p.ND + 1;
                        blk_end   = m*2*p.ND;

                        % Transmit block [sounding; data]
                        x_block = [p.s_mls; p.s_mls * d_sim(m)];   % 2ND x 1
                        ...
                        % Apply delay (circular shift within the block)
                        x_del_DU  = circshift(x_block, ell_du);   % 2ND x 1
                        x_del_DTU = circshift(x_block, ell_dtu);  % 2ND x 1

                        y_rx_flat(blk_start:blk_end) = ...
                            A_DU_m(m)  * x_del_DU  * ph_DU  ...
                          + A_DTU_m(m) * x_del_DTU * ph_DTU ...
                          + sigU * (randn(2*p.ND,1) + 1j*randn(2*p.ND,1));
                    end

                    % ---- 4e. Frame synchronisation: compensate direct-path delay ----
                    % Use average (or initial) ell_DU as the fixed sync offset
                    ell_sync = mob.ell_DU_seq(1);
                    y_rx_flat_aligned = circshift(y_rx_flat, [-ell_sync, 0]);

                    % ---- 4f. Reshape to 2ND x M_seq matrix ----
                    y_full_sim = reshape(y_rx_flat_aligned, [2*p.ND, obj.M_seq]);
                    yS = y_full_sim(1:p.ND,     :);   % ND x M_seq sounding
                    yP = y_full_sim(p.ND+1:end, :);   % ND x M_seq data
